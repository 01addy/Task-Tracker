...
export const createAndSendOtp = async (email, purpose = "signup") => {
  ...
  sendOtpEmail(email, otp, OTP_EXPIRE_MIN).then((r) => {
    if (!r.ok) console.error("Background sendOtpEmail failed:", r.error);
  }).catch((err) => {
    console.error("sendOtpEmail (background) exception:", err);
  });
